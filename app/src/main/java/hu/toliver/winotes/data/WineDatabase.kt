package hu.toliver.winotes.data

import androidx.room.Database
import androidx.room.RoomDatabase
import androidx.room.TypeConverters
import hu.toliver.winotes.data.dao.SyncMetadataDao
import hu.toliver.winotes.data.dao.TasteDao
import hu.toliver.winotes.data.dao.WineDao
import hu.toliver.winotes.data.local.converters.DateConverter
import hu.toliver.winotes.data.local.converters.ListConverter
import hu.toliver.winotes.data.local.entity.SyncMetadataEntity
import hu.toliver.winotes.data.local.entity.TasteEntity
import hu.toliver.winotes.data.local.entity.WineEntity

@Database(
    entities = [
        WineEntity::class,
        TasteEntity::class,
        SyncMetadataEntity::class
    ],
    version = 1,
    exportSchema = true
)
@TypeConverters(DateConverter::class, ListConverter::class)
abstract class WineDatabase : RoomDatabase() {
    abstract fun wineDao(): WineDao
    abstract fun tasteDao(): TasteDao
    abstract fun syncMetadataDao(): SyncMetadataDao
}